...
def populate(request):
    ...
    try:
        with psycopg2.connect(host='127.0.0.1',
                                ...
                                password='secret') as conn:
            with conn.cursor() as cur:
                command = 'INSERT INTO ex04_movies (episode_nb, title, director, producer, release_date) '
...
def display(request):
    ...
    try:
        with psycopg2.connect(host='127.0.0.1',
                                ...
                                password='secret') as conn:
            with conn.cursor() as cur:
                command = 'SELECT * FROM ex04_movies;'
                cur.execute(command)
                movies = cur.fetchall()
                if len(movies) == 0:
                    raise Exception()
    ...
    if request.method == 'GET':
        ...
        try:
            with psycopg2.connect(host='127.0.0.1',
                                  ...
                                  password='secret') as conn:
                with conn.cursor() as cur:
                    command = 'SELECT title FROM ex04_movies;'
                    cur.execute(command)
                    titles = cur.fetchall()
                    if len(titles) == 0:
                        raise Exception()
        ...
        try:
            with psycopg2.connect(host='127.0.0.1',
                                  ...
                                  password='secret') as conn:
                with conn.cursor() as cur:
                    command = 'DELETE FROM ex04_movies WHERE title = %s ;'
                    cur.execute(command, [request.POST.get('remove')])
                    conn.commit()
